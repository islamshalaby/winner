...
@section('content')
    <div class="col-lg-12 col-12 layout-spacing">
        ...
        <form action="" method="post" enctype="multipart/form-data" >
            ...
            <div class="form-group mb-4  outside-div {{ $data['ad']['type'] == 'inside' ? 'hidden-div' : '' }}">
                <label for="link">{{ __('messages.link') }}</label>
                <input  type="text" name="contentLink" class="form-control" id="link" placeholder="{{ __('messages.link') }}" value="{{ $data['ad']['content'] }}" >
            </div>

            <div class="form-group mb-4 inside-div {{ $data['ad']['type'] == 'inside' ? '' : 'hidden-div' }}">
                <label for="link">{{ __('messages.select_product') }}</label>
                <select  name="contentId" class="form-control">
                    <option selected disabled >{{ __('messages.select') }}</option>
                    @foreach ( $data['products'] as $product )
                        <option {{ $data['ad']['content'] == $product->id ? 'selected' : '' }} value="{{ $product->id }}" >{{ $product->title }}</option>
                    @endforeach                
                </select> 
            </div>
